...
public class Solver {
  ...
  private int numAssignedCustomers;
  private double totalDemand, totalCapacity;
  private double demandFactor;
  private int newWarehouseMode = 2;
  ...
  */
  public Solver(String[] args) throws IOException {
    ...
    // initial state: all warehouses are closed and no customers are assigned
    
    /*
    *     a greedy solution
    */
   
    // open enough warehouses to meet the total demand
    totalDemand = 0;
    totalCapacity = 0;
    for (int i = 0; i < M; i++) totalDemand += customers[i].demand();
    while (totalCapacity < totalDemand) {
      int windex = openNewWarehouse(newWarehouseMode);
      totalCapacity += warehouses[windex].capacity();
    }
    
    
    // construct a greedy feasible solution
    //java.util.Arrays.sort(customers);
    numAssignedCustomers = 0;
    while (numAssignedCustomers < M) {
      clearWarehouses();
      assignCustomers();
      openSwap();
      if (numAssignedCustomers < M) {
        int windex = openNewWarehouse(newWarehouseMode);
        totalCapacity += warehouses[windex].capacity(); 
      }       
    }
...
*/    
  ...
  private void openUncap(int K) {
    // opens K currently closed warehouses in order of lowest total uncapacitated cost
    ...
    for (int k = 0; k < K; k++) {
      ...
      int minw = -1;
      for (int j = 0; j < N; j++) {
        if (warehouses[j].open()) continue;
        double sum = warehouses[j].setup();
        for (int i = 0; i < M; i++) sum += customers[i].cost(j);
        if (sum < min) {
          min = sum;
          minw = j;
        }
      ...
      warehouses[minw].setOpen();      
    }
    
  }
  
  private void closedSwap() {
    // swaps an open warehouse with a closed warehouse
  ...
  private void openSwap() {
    // swaps the customers served by two open warehouses
    Bag<Integer> openWarehouses = new Bag<Integer>();
    for (int j = 0; j < N; j++) {
      if (warehouses[j].open()) openWarehouses.add(j);
    }
  ...
  private int openNewWarehouse(int mode) {
    ...
    int windex = -1;
    if      (mode == 1) windex = w1;
    else if (mode == 2) windex = w2;
    else                windex = w3;
    if (windex != -1 ) warehouses[windex].setOpen();
    return windex;
  }
  ...
  */
  private class Customer implements Comparable<Customer> {
    ...
    public int compareTo(Customer that) {
      // results in ordering by decreasing demand
